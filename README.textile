...
Validate attachment's MIME type:

<pre><code>var $validate = array(
	'avatar' => array(
		'image1 => array(
			'rule' => array('attachmentMimeType', 'image/jpeg'),
			'message' => 'Only jpegs please'
		),
		'image2' => array(
			'rule' => array('attachmentMimeType', array('image/jpeg', 'image/gif')),
			'message' => 'Only jpegs or gifs please'
		),
	)
);
...
This is forked from https://github.com/szajbus/uploadpack .
Because of current uploadpack is only support CakePHP2.x and we need for 1.3.
